...
2. 인공지능 학습
3. 데이터 베이스 연동
(시스템 밀접한 프로그래밍이나, 모바일 프로그래밍엔 부적합)
'''

#파이썬의 특정 부분의 구별: 들여쓰기 (C언어는 세미콜론';')

### <파이썬 자료형>

## 파이썬 숫자형 ----------@
#파이썬 숫자형 e,o,x의 대소문자 구분 없습니다.
'''
a1 = 1234
print(a1)
a2 = 123.4
...
#파이썬 제공 내장함수: format() 리턴타입은 문자열 입니다.
b = format(value, '#b')
o = format(value, '#o')
h = format(value, '#x')
print(b)
...
#다른 진수형태에서 10진수로 반환 리턴타입은 숫자, 파라미터는 문자열과 변환되는 대상의 진수형태 입니다.
b = int(b, 2)
o = int(o, 8)
h = int(h, 16)
print(b)
...
print(len(A))
print(A.startswith('Li')) #참이면 True 반환
print(A.endswith('thon'))
print(A.find('py')) #참이면 시작인덱스, 거짓이면 -1 반환
...
##파이썬 순서형 ----------
'''
#list와 tuple (문자열도 순서형 sequential type)
# 순서형에서 사용하는 연산으로 인덱스(L[0]), 슬라이싱(L[0:]), 길이반환(len()),
# 연결(+), 반복(*), 멤버검사 (in 순서형)이 있다.
